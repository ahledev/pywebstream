...
plateDetector = PlateDetector(type_of_plate='RECT_PLATE', minPlateArea=4100, maxPlateArea=15000)
...
def recognized_plate(temp, size):
    ...
    plate, len_plate = myNetwork.label_image_list(temp[1], size)
    ...
    print("recognized plate: " + recog_plate)
...
def alpr_on_image(frame):
    global coordinates, list_char_on_plate, countPlates, recog_plate
    ...
    if possible_plates is not None:
        ...
        threading.Thread(target=recognized_plate, args=(temp, 128)).start()

        if (distance < 100):
            #if (countPlates < countPlates_threshold):
                # cv2.imshow('Plate', possible_plates[0])
                # print(possible_plates[0])
                #temp = []
                #temp.append(possible_plates[0])
                #temp.append(plateDetector.char_on_plate[0])  # temp = [image of plate, segmented characters on plate]
...
def gen():
    while True:
        ...
        if (frame is None):
            ...
            break
        if(cap.isOpened()):
            alpr_frame = alpr_on_image(frame)
        ret, jpeg = cv2.imencode(".jpg", alpr_frame)
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + jpeg.tobytes() + b'\r\n')

def genPlate():
    while True:
        ret, jpeg = cv2.imencode(".jpg", recog_plate)
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + jpeg.tobytes() + b'\r\n')
